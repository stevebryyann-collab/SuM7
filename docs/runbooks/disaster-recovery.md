...
2. **Stop the bleeding.** If a bad deploy: roll back first (see _Bad deploy_). If data corruption:
...
### External providers (Shopify / Paddle / Clerk / Resolve)
...
- Reconcile financial state: invoice sequence continuity, AR totals, Paddle transaction events vs DB.
