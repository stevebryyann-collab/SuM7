...
| `STRIPE_SECRET_KEY` / `STRIPE_WEBHOOK_SECRET` | API | Railway, Stripe dashboard |
...
### `*_WEBHOOK_SECRET` (Clerk / Stripe / Resolve)
...
### `CLERK_SECRET_KEY` / `STRIPE_SECRET_KEY` / `RESOLVE_API_KEY` / `RESEND_API_KEY`
