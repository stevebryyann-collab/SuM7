  ...
  return (
    <div className="panel p-8">
      ...
      <form onSubmit={start} className="space-y-4">
        ...
        {hasError ? (
          <p className="text-sm text-red-700">
            Sign-in failed. Please try connecting your store again.
          </p>
        ) : null}
        <Button
          type="submit"
          variant="primary"
          className="w-full"
          disabled={loading || shop.trim().length === 0}
        >
      ...
      <p className="mt-3 text-center text-xs text-text-tertiary">
        ...
        <a href="/terms" className="hover:text-accent hover:underline">
          Terms of Service
        </a>
      </p>
...
export default function MerchantLoginPage(): JSX.Element {
  return (
    <Suspense
      fallback={
        <div className="panel p-8 text-sm text-text-secondary">Loading…</div>
      }
    >
      <MerchantLoginForm />
    </Suspense>
