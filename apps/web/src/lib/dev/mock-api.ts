 ...
 */
export async function mockMerchantRequest<T>(
  path: string,
  ...
  if (url === "/buyers" && method === "GET") {
    ...
    const tierName = tierId
      ? (DEMO_PRICING_TIERS.find((t) => t.id === tierId)?.name ?? null)
      : null;
    let rows: BuyerSummary[] = [...DEMO_BUYERS];
    if (search)
      rows = rows.filter((b) => b.companyName.toLowerCase().includes(search));
  ...
  if (
    ...
    if (removed && url.endsWith("/approve")) {
      ...
      const tierName = dto.pricingTierId
        ? (DEMO_PRICING_TIERS.find((t) => t.id === dto.pricingTierId)?.name ??
          null)
        : null;
  ...
  if (
    method === "POST" &&
    /^\/buyers\/applications\/[^/]+\/reveal$/.test(url)
  ) {
    const appId = segment(url, 2);
    const pii: ApplicationPii = DEMO_APPLICATION_PII[appId] ?? {
      taxId: null,
      phone: null,
    };
    return pii as T;
  ...
  if (method === "POST" && /^\/buyers\/[^/]+\/reinstate$/.test(url)) {
    ...
    const current =
      detail?.approvalStatus ?? summary?.approvalStatus ?? "unknown";
  ...
  if (url === "/orders" && method === "GET") {
    ...
    const buyerName = buyerId
      ? (DEMO_BUYERS.find((b) => b.buyerId === buyerId)?.companyName ?? null)
      : null;
    ...
    if (search) {
      rows = rows.filter((o) =>
        (o.shopifyOrderNumber ?? "").toLowerCase().includes(search),
      );
    }
    if (dateFrom) rows = rows.filter((o) => o.createdAt >= dateFrom);
    if (dateTo)
      rows = rows.filter((o) => o.createdAt <= `${dateTo}T23:59:59.999Z`);
    return paginate(rows, cursor) as T;
  }
  ...
  if (url === "/invoices" && method === "GET") {
    ...
    if (search)
      rows = rows.filter((i) => i.invoiceNumber.toLowerCase().includes(search));
    return paginate(rows, cursor) as T;
  }

  if (method === "PATCH" && /^\/invoices\/[^/]+\/mark-paid$/.test(url)) {
    const id = segment(url, 1);
    const dto = options.body as {
      amount?: string;
      reference?: string;
      paidAt?: string;
    };
  ...
  if (method === "PATCH" && /^\/invoices\/[^/]+\/void$/.test(url)) {
    const id = segment(url, 1);
    const reason =
      (options.body as { reason?: string } | undefined)?.reason ??
      "Voided in demo";
  ...
  if (url === "/api/v1/pricing-tiers" && method === "GET") {
    ...
    const withDetails: PricingTierSummary[] = DEMO_PRICING_TIERS.map(
      (tier) => ({
        ...tier,
  ...
  if (url === "/api/v1/pricing-tiers" && method === "POST") {
    ...
    const summary: PricingTierSummary = {
      ...
      type: dto.type,
      baseDiscountPct:
        dto.baseDiscountPct !== undefined
          ? dto.baseDiscountPct.toFixed(2)
          : null,
  ...
  if (
    method === "POST" &&
    /^\/api\/v1\/pricing-tiers\/[^/]+\/overrides\/bulk$/.test(url)
  ) {
    const tierId = segment(url, 3);
    if (!DEMO_PRICING_TIERS.some((t) => t.id === tierId))
      return notFound(`POST ${url}`);
    const dto = options.body as BulkPricingOverrideInput;
    const list =
      DEMO_TIER_OVERRIDES[tierId] ?? (DEMO_TIER_OVERRIDES[tierId] = []);
    for (const o of dto.overrides) {
      const variantKey = o.shopifyVariantId ?? "";
      const existing = list.find(
        (x) =>
          x.shopifyProductId === o.shopifyProductId &&
          (x.shopifyVariantId ?? "") === variantKey,
      );
      if (existing) {
  ...
  if (
    method === "DELETE" &&
    /^\/api\/v1\/pricing-tiers\/[^/]+\/overrides\/[^/]+$/.test(url)
  ) {
    const tierId = segment(url, 3);
  ...
  if (method === "PATCH" && /^\/api\/v1\/pricing-tiers\/[^/]+$/.test(url)) {
    ...
    if (dto.baseDiscountPct !== undefined) {
      tier.baseDiscountPct =
        dto.baseDiscountPct === null ? null : dto.baseDiscountPct.toFixed(2);
    }
    if (dto.isDefault !== undefined) tier.isDefault = dto.isDefault;
    if (dto.minOrderAmount !== undefined)
      tier.minOrderAmount = dto.minOrderAmount;
    ...
    if (dto.conditionsJson !== undefined)
      DEMO_TIER_CONDITIONS[tierId] = dto.conditionsJson ?? null;
    return undefined as T;
  }
  ...
  if (method === "DELETE" && /^\/api\/v1\/pricing-tiers\/[^/]+$/.test(url)) {
    ...
    if (tier && tier.buyerCount > 0) {
      conflict(
        "TIER_HAS_ACTIVE_BUYERS",
        "Cannot delete a tier with assigned buyers",
      );
    }
  ...
  if (url === "/api/v1/dashboard" && method === "GET") {
    ...
    return {
      ...
      kpis: {
        ...DEMO_DASHBOARD_DATA.kpis,
        pendingApplicationCount: DEMO_APPLICATIONS.length,
      },
    } as T;
  }
  ...
  if (url === "/api/v1/analytics/export" && method === "GET") {
    ...
    if (type === "gdpr") {
      return {
        ...
        type: "gdpr",
        message:
          "Your data export is being prepared. We will email you when it is ready.",
      } as T;
    }
  ...
  if (method === "PATCH" && /^\/api\/v1\/team\/[^/]+$/.test(url)) {
    ...
    if (member.role === "owner")
      conflict("CANNOT_MODIFY_OWNER", "The owner role cannot be changed");
  ...
  if (method === "DELETE" && /^\/api\/v1\/team\/[^/]+$/.test(url)) {
    ...
    if (member.role === "owner")
      conflict("CANNOT_REMOVE_OWNER", "The owner cannot be removed");
  ...
  if (url === "/api/v1/billing/change-plan" && method === "POST") {
    ...
    const wasTrial = DEMO_BILLING_PLAN.isTrial;
    applyDemoPlanChange(dto.tier);
    // First subscription (from trial) → Paddle returns a hosted-checkout URL;
    // an existing subscriber gets an in-place, immediately-prorated tier change.
    if (wasTrial) {
      return {
        checkoutUrl:
          "https://sandbox-checkout.paddle.com/checkout?_ptxn=txn_demo_0001",
      } as T;
    }
    return { tier: dto.tier } as T;
  }
  if (url === "/api/v1/billing/create-portal-session" && method === "POST") {
    return {
      url: "https://sandbox-customer-portal.paddle.com/session/demo",
    } as T;
  }
  ...
  if (url === "/rep/sessions" && method === "POST") {
    const buyerId =
      (options.body as { buyerId?: string } | undefined)?.buyerId ?? "";
  ...
  if (
    method === "DELETE" &&
    /^\/api\/v1\/data-export\/gdpr\/[^/]+$/.test(url)
  ) {
...
function analyticsExportCsv(type: string): string {
  ...
  if (type === "invoices") {
    lines = [
      "Invoice Number,Buyer,Status,Total,Amount Paid,Due Date,Created At",
    ];
    for (const i of DEMO_INVOICES) {
      lines.push(
        [
          cell(i.invoiceNumber),
          cell(i.buyerCompanyName ?? ""),
          i.status,
          i.total,
          i.amountPaid,
    ...
    for (const o of DEMO_ORDERS) {
      lines.push(
        [
          cell(o.shopifyOrderNumber ?? ""),
          cell(o.buyerCompanyName ?? ""),
          o.status,
          o.total,
          o.currency,
          o.createdAt,
...
const DEMO_TIER_PRICE: Record<string, string> = {
  starter: "29.00",
  growth: "79.00",
  pro: "199.00",
};
const DEMO_TIER_FREE: Record<string, string> = {
  starter: "20000.00",
  growth: "50000.00",
  pro: "150000.00",
};
const DEMO_TIER_RATE: Record<string, number> = {
  starter: 0.005,
  growth: 0.004,
  pro: 0.003,
};
...
function applyDemoPlanChange(tier: string): void {
  ...
  const billable = Math.max(
    Number(DEMO_BILLING_USAGE.gmvCurrentMonth) - Number(free),
    0,
  );
  DEMO_BILLING_USAGE.tier = tier;
  DEMO_BILLING_USAGE.freeThreshold = free;
  DEMO_BILLING_USAGE.billableGmv = billable.toFixed(2);
  DEMO_BILLING_USAGE.estimatedFee = (
    billable * (DEMO_TIER_RATE[tier] ?? 0.005)
  ).toFixed(2);
}
...
function markInvoicePaid(
  ...
  const newPaid = Math.min(
    prevPaid + (Number.isFinite(payment) ? payment : 0),
    total,
  );
  ...
  if (detail) {
    ...
    detail.auditTrail = [
      {
        id: `aud-${id}-paid-${detail.payments.length}`,
        action: "paid",
...
function voidInvoice(
  ...
  if (summary?.status === "paid" || summary?.status === "void") {
    conflict(
      ...
      `Invoice is ${summary.status} and cannot be voided`,
    );
  }
...
function sendInvoiceReminder(id: string): {
  sent: boolean;
  ...
  const detail = DEMO_INVOICE_DETAILS[id];
  if (!summary && !detail)
    return notFound(`POST /invoices/${id}/send-reminder`);
  const current = summary?.reminderCount ?? detail?.reminderCount ?? 0;
  if (current >= 3) {
    conflict(
      "REMINDER_LIMIT_REACHED",
      "The maximum of 3 reminders has already been sent",
    );
  }
...
function arAgingCsv(): string {
  const rows: Array<[string, number, string]> = [
    [
      "Current (not yet due)",
      DEMO_AR_AGING.current.invoiceCount,
      DEMO_AR_AGING.current.outstandingAmount,
    ],
    [
      "1–30 days overdue",
      DEMO_AR_AGING.overdue_1_30.invoiceCount,
      DEMO_AR_AGING.overdue_1_30.outstandingAmount,
    ],
    [
      "31–60 days overdue",
      DEMO_AR_AGING.overdue_31_60.invoiceCount,
      DEMO_AR_AGING.overdue_31_60.outstandingAmount,
    ],
    [
      "61–90 days overdue",
      DEMO_AR_AGING.overdue_61_90.invoiceCount,
      DEMO_AR_AGING.overdue_61_90.outstandingAmount,
    ],
    [
      "90+ days overdue",
      DEMO_AR_AGING.overdue_90_plus.invoiceCount,
      DEMO_AR_AGING.overdue_90_plus.outstandingAmount,
    ],
  ];
  ...
  for (const [label, count, amount] of rows)
    lines.push(`${cell(label)},${count},${amount}`);
 ...
 */
export async function mockMerchantGraphQL<TData>(
  document: string,
): Promise<TData> {
  await delay(120);
