...
export function buildAuthOptions(): NextAuthOptions {
  ...
  const shopifyClientId = isDemoEnabled() ? process.env.SHOPIFY_CLIENT_ID : serverEnv('SHOPIFY_CLIENT_ID');
  ...
  if (shopifyClientId && shopifyClientSecret) {
    providers.push(
      ShopifyProvider({
        ...
        scopes: process.env.SHOPIFY_SCOPES ?? 'read_products,write_orders,read_orders,read_customers',
      }),
    );
  }
  ...
  // Hardened cookies: __Secure- prefix + Strict in production.
  ...
  callbacks: {
    ...
    async signIn({ account, profile, user }) {
      ...
      if (account?.provider === DEMO_PROVIDER_ID && isDemoEnabled()) return true;
    ...
    async jwt({ token, user }) {
      ...
      const fresh = (user as unknown as Record<string, unknown> | undefined)?.merchantClaims as
        | MerchantClaims
        | undefined;
      if (fresh) {
