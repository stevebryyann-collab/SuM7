...
const NAV_GROUPS: { label: string; items: NavItemDef[] }[] = [
  ...
  { label: 'Insights', items: [{ href: '/analytics', label: 'Analytics', icon: BarChart2 }] },
  {
    ...
    items: [
      ...
      { href: '/settings/health', label: 'System Health', icon: Activity, roles: ['owner', 'admin'] },
    ],
  },
];
          ...
          <div key={group.label}>
            ...
            {group.items.map((item) => (
              <NavItem
                ...
                justIncreased={item.badge === 'pendingBuyers' ? pendingPulse : false}
              />
            ))}
          </div>
        ))}
...
function NavItem({
  ...
  return (
    ...
    >
      {active ? (
        <span className="absolute left-0 top-1/2 h-5 w-1 -translate-y-1/2 rounded-r-full bg-ocean shadow-glow" aria-hidden />
      ) : null}
      <Icon className={cn('h-4 w-4', active ? 'text-ocean' : 'text-text-tertiary')} />
