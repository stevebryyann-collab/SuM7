...
export interface AppConfig {
  ...
  STRIPE_GMV_METERED_PRICE_ID: string;
...
export const envValidationSchema = Joi.object({
  ...
  // ── Redis ──
  REDIS_CACHE_URL: Joi.string().uri({ scheme: ['redis', 'rediss'] }).required(),
  REDIS_QUEUE_URL: Joi.string().uri({ scheme: ['redis', 'rediss'] }).required(),
  ...
  // ── Stripe ──
  STRIPE_SECRET_KEY: Joi.string().pattern(/^sk_/, 'stripe secret key').required(),
  STRIPE_WEBHOOK_SECRET: Joi.string().pattern(/^whsec_/, 'stripe webhook secret').required(),
  STRIPE_STARTER_PRICE_ID: Joi.string().required(),
  STRIPE_GROWTH_PRICE_ID: Joi.string().required(),
  STRIPE_PRO_PRICE_ID: Joi.string().required(),
  STRIPE_GMV_METERED_PRICE_ID: Joi.string().required(),
  ...
  RESOLVE_API_BASE_URL: Joi.string().uri().default('https://app.resolvepay.com/api'),
