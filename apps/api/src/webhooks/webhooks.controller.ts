...
export class WebhooksController {
  constructor(
    ...
    @InjectQueue(QUEUE_INVOICE) private readonly invoiceQueue: Queue<WebhookJobData>,
    @InjectQueue(QUEUE_ORDER) private readonly orderQueue: Queue<WebhookJobData>,
    @InjectQueue(QUEUE_CATALOG) private readonly catalogQueue: Queue<WebhookJobData>,
    @InjectQueue(QUEUE_BUYER) private readonly buyerQueue: Queue<WebhookJobData>,
    @InjectQueue(QUEUE_MERCHANT) private readonly merchantQueue: Queue<WebhookJobData>,
  ) {}

  @Post('orders/created')
  @HttpCode(200)
  ordersCreated(@Req() req: ShopifyWebhookRequest): Promise<WebhookAck> {
    return this.ingest(req, 'orders/create', this.invoiceQueue, JOB_INVOICE_GENERATE, 1);
  }

  @Post('orders/updated')
  @HttpCode(200)
  ordersUpdated(@Req() req: ShopifyWebhookRequest): Promise<WebhookAck> {
    return this.ingest(req, 'orders/updated', this.orderQueue, JOB_ORDER_SYNC, 5);
  }
  ...
  productsCreated(@Req() req: ShopifyWebhookRequest): Promise<WebhookAck> {
    return this.ingest(req, 'products/create', this.catalogQueue, JOB_CATALOG_SYNC, 10);
  }

  @Post('products/updated')
  @HttpCode(200)
  productsUpdated(@Req() req: ShopifyWebhookRequest): Promise<WebhookAck> {
    return this.ingest(req, 'products/update', this.catalogQueue, JOB_CATALOG_SYNC, 10);
  }

  @Post('products/deleted')
  @HttpCode(200)
  productsDeleted(@Req() req: ShopifyWebhookRequest): Promise<WebhookAck> {
    return this.ingest(req, 'products/delete', this.catalogQueue, JOB_CATALOG_SYNC, 10);
  }

  @Post('customers/create')
  @HttpCode(200)
  customersCreate(@Req() req: ShopifyWebhookRequest): Promise<WebhookAck> {
    return this.ingest(req, 'customers/create', this.buyerQueue, JOB_BUYER_SYNC, 10);
  }

  @Post('app/uninstalled')
  @HttpCode(200)
  appUninstalled(@Req() req: ShopifyWebhookRequest): Promise<WebhookAck> {
    return this.ingest(req, 'app/uninstalled', this.merchantQueue, JOB_MERCHANT_CLEANUP, 1);
  }
  ...
  private async ingest(
    ...
    const shopifyEventId = typeof eventIdHeader === 'string' ? eventIdHeader : '';
    const createdAtHeader = req.headers['x-shopify-webhook-created-at'];
    const shopifyCreatedAt =
      typeof createdAtHeader === 'string' && !Number.isNaN(Date.parse(createdAtHeader))
